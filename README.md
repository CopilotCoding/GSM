...
Trained on 228 Bach MIDI files in under an hour on consumer hardware. Generates convincing baroque piano music.
...
The model maintains a single fixed-size point `S ∈ R^N` — a position on a high-dimensional geometric object. Each token is not a data point to store but a **transformation operator** that deforms that geometry. The state is never a memory buffer being overwritten. It's a manifold position being continuously reshaped by a learned transformation algebra.
...
The subspace rotation component has no RNN analogue at all. It applies learned rotations in random fixed dimension pairs across R^N — pure geometric deformation with no linear algebra equivalent in classical sequence models.
...
This is what makes GSM not just a fast RNN. There is no classical sequence model operation that corresponds to input-parameterized subspace rotations on a fixed geometric object.
...
Trained on 228 Bach MIDI files (217 processed), 32M parameters, RTX 5060 Ti, ~45 minutes.
...
At temperature 0.75 after ~47 epochs: generates convincing baroque piano music in the style of Bach.
